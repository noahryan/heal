...
--data StackOp a = StackOp { stackeffect::Int, needsonstack::Int, applyOp::[a] -> [a], name::String }

rgepEval :: S.Seq (OP b) -> Maybe b
rgepEval = postfix
--rgepEval = postfix . V.reverse -- . uncode

rgeprecomb pm pr pc1 pc2 pop = 
  mutation pm pop >>= rotation pr >>= 
  crossover 1 pc1 >>= crossover 2 pc2

rgepea ps is ops terms pm pr pc1 pc2 opt eval gens = 
  ea (mkCodonPop ps is (codonLength ops terms))
...
cdn2int bs = F.sum $ S.zipWith (\b i -> if b then i else 0) bs is where
  is = S.fromList [2^i | i <- [1..S.length bs-1]]
